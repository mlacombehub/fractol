...
/*   Updated: 2020/07/16 09:48:36 by mlacombe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		hook(int key, t_fol_t *fol)
{
	if (fol->tom_hook == 1)
	{
		key == KEY_C ? fol->fractal.color -= 0x00000001 : 42;
		key == KEY_X ? fol->fractal.color -= 0x00000100 : 42;
		key == KEY_Z ? fol->fractal.color -= 0x00010000 : 42;
	}
	else
	{
	...
	key == KEY_Q ? fol->fractal.set.new_c.r += 0.01 : 42;
	key == KEY_A ? fol->fractal.set.new_c.r -= 0.01 : 42;
	key == KEY_W ? fol->fractal.set.new_c.i += 0.01 : 42;
	key == KEY_S ? fol->fractal.set.new_c.i -= 0.01 : 42;
...
int		key_hook(int key, t_fol_t *fol)
{
	...
	if (key == KEY_N1 || key == KEY_N2 || key == KEY_N3 || key == KEY_ENTER)
	{
		fol->f_type = 0;
		if (key == KEY_N1)
			ft_memcpy(fol->progname, "mandelbrot", 15);
		if (key == KEY_N2)
			ft_memcpy(fol->progname, "julia", 15);
		if (key == KEY_N3)
			ft_memcpy(fol->progname, "burningship", 15);
	}
	...
	if (key == KEY_U && fol->fractal.i_max > 10 && fol->fractal.i_max <= 50)
		...
		fol->fractal.i_max -= 50;
	key == KEY_SHIFT ? (fol->tom_hook = 1) : 42;
	key == KEY_CTRL ? (fol->tom_hook = 0) : 42;
	fol_select(fol);
	return (0);
}
